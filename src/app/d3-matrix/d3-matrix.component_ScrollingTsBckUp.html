...
export class D3MatrixComponent implements OnInit, OnChanges {
    ...
    generateVisual(data) {
        ...
        var abc = this.NextPrevgridData(gridData)    
        ...
        content.addEventListener('scroll', function (evt) {
            ...
            ////////
            console.log(abc)
            var roww = grid.selectAll(".rowd33")
            .data(abc)
            .enter().append("g")
            .attr("class", "rowd33");

            var NextPrevColumn = roww.selectAll(".square")
            .data(function (d: any) { return d; })
            .enter().append("rect")
            //.attr("id", function (abc: any, i) { return "rect-" + abc.y + i })

            var NextPrevColumnAttributes = NextPrevColumn
            .attr("x", function (abc:any) {
                //console.log(abc)
                 return abc.x; })
            .attr("y", function (abc: any) { return abc.y; })
            .attr("rx", "5")
            .attr("ry", "5")
            .attr("width", function (abc: any) { return abc.width })
            .attr("height", function (abc: any) { return abc.height })
            .attr("stroke", "#66a877")//"#acb7b7"
            .attr("fill",
                function (d: any) {
                    // //console.log(d.positionalPairOfApi)
            ...
            if(!document.getElementById(abc[0][0].hindiIndexWise) ){
                console.log(abc[0].hindiIndexWise)
        var labelnext: any = labell.append("text")
            .attr("x", "36")
            .attr("y", function (d, i) {
                //return gridData[36][0].y + 34;
                return d[0].y + 17;
            })
            .style("font-size", "16px")
            .attr("id", function (d, i) {
        ...
        for (var i = 0; i < labelnext.nodes().length; i++) {
            //console.log(  label.nodes()[i].getComputedTextLength());
            var textLen = labelnext.nodes()[i].getComputedTextLength();
            labelnext.nodes()[i].setAttribute('x', 175 - textLen)
        }
    }
    ...
    NextPrevgridData(d: any) {
        console.log(d)
        let columncount = d.length
        var xpos = 200;
        var ypos = d[columncount - 1][0].y + 25 ;
        var width = 25;
        var height = 25;       
   
         //d.englishword;
        let hindiVerticalWords = ["अब्राहम", "से", "दाऊद", "तक", "सब", "चौदह", "पीढ़ी", "हुई", "और", "दाऊद", "से", "बाबेल", "को", "बन्दी", "होकर", "पहुँचाए", "जाने", "तक", "चौदह", "पीढ़ी", "और", "बन्दी", "होकर", "बाबेल", "को", "पहुँचाए", "जाने", "के", "समय", "से", "लेकर", "मसीह", "तक", "चौदह", "पीढ़ी", "हुई"];
